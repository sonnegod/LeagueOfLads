...
async function runAllLeagues() {
  for (const league of db.preloadedData.leagues) {
    const result = await checkMatches(league.LeagueId);
    console.log(`Done with league ${league.LeagueId}, ${result.totalNewMatches} new matches.`);
  }
}
